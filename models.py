...
class FrameEncoder(nn.Module):
    ...
    def forward(self, x, x_mask, attn):
        # expand phone to frame
        dataType = x.dtype
        x = torch.matmul(attn.squeeze(1), x.transpose(1, 2)).transpose(1, 2)
        x_mask = torch.matmul(attn.squeeze(1), x_mask.transpose(1, 2)).transpose(1, 2)
        x = x.to(dataType)
        x_mask = x_mask.to(dataType)

        # pytorch tensor are not reversible, hence the conversion
        x_lengths = x_mask.sum(2)
        x_lengths = x_lengths.squeeze(1).int()
        x_lengths = x_lengths.cpu().numpy()
        ...
        m_frame, logs_frame = torch.split(stats, 192, dim=1)
        return m_frame, logs_frame
...
class SynthesizerTrn(nn.Module):
    ...
    def infer_lstm(
        ...
        z_p = m_frame + torch.randn_like(m_frame) * torch.exp(logs_frame) * noise_scale
        z = self.flow(z_p, y_mask, g=g, reverse=True)
        o = self.dec((z * y_mask)[:, :, :max_len], g=g)
        return o, attn, y_mask, (z, z_p, m_p, logs_p)
